  ...
  return (
    ...
    <div className="panel-scope flex min-h-screen bg-paper text-ink md:h-screen md:overflow-hidden">
      {/* Mobil üst çubuk. Artık bir hamburger taşımıyor — gezinme ekranın
          ...
          Zemini `.app-bar` veriyor (globals.css): sayfanın kendi rengi, buzlu.
          İçerik altından kayıp geçtiğinde çubuk onu bulanıklaştırarak taşıyor —
          yerli uygulamalarda üst çubuğun ayrı bir bant değil, ekranın devamı
      ...
      <header className="app-bar fixed inset-x-0 top-0 z-30 flex items-center justify-between px-4 md:hidden">
        ...
        <div className="flex min-w-0 items-center gap-2.5">
          <Link href="/admin" className="tab-press flex shrink-0 items-center">
        ...
        <div className="flex shrink-0 items-center gap-1">
          <Link
            ...
            className="tab-press relative flex h-10 w-10 items-center justify-center rounded-full text-ink-muted"
          >
      ...
      {/* Kenar çubuğu yalnızca masaüstünde. Mobilde onun yerini `MobileTabBar`
          ...
          Sedef'te ray koyu değil: içerik alanıyla aynı ışıkta duran, ondan
          yalnızca bir ton ve ince bir çizgiyle ayrılan bir yüzey. Seçili
          sayfayı zeminin koyuluğu değil, vurgu renkli hap işaret ediyor. */}
      <aside className="sidebar relative z-auto hidden w-64 shrink-0 flex-col justify-between border-r border-border bg-surface transition-[width] duration-200 ease-out md:flex">
        <div>
        ...
        <div className="sidebar-foot mt-auto border-t border-border p-6">
...
function UnreadDot({ promise }: { promise: Promise<number> }) {
  ...
  return (
    <span
      ...
      className="absolute right-2 top-2 h-2.5 w-2.5 rounded-full bg-accent ring-2 ring-paper"
    />
  );
}
...
function YonetimLink({ promise }: { promise: Promise<Identity> }) {
  ...
  return (
    <Link
      href="/yonetim"
      className="mb-4 flex items-center gap-3 rounded-md px-2 py-2 text-sm font-medium text-ink-muted transition hover:bg-accent-soft hover:text-accent-hover"
    >
...
const avatarClass =
  "flex items-center justify-center rounded-full bg-accent font-semibold text-white shadow-[0_6px_14px_-8px_var(--color-accent)]";
...
function SidebarToggle() {
  return (
    <button
      ...
      className="sidebar-toggle icon-btn hidden h-8 w-8 shrink-0 md:inline-flex"
    >
...
function GreetingFallback() {
  return <span className="block h-4 w-32 animate-pulse rounded-full bg-border" />;
}
