/** Türkiye il ve ilçe listesi (81 il, 973 ilçe), Türkçe alfabetik sırada. */
...
/** Bir ilin ilçeleri; il bulunamazsa boş dizi. */
export function districtsOf(province: string): readonly string[] {
  return PROVINCES.find((p) => p.name === province)?.districts ?? [];
}

export function isProvince(province: string): boolean {
  return PROVINCES.some((p) => p.name === province);
}

/** İlçenin gerçekten o ile ait olup olmadığı — seçim listesi atlanırsa diye. */
export function isDistrictOf(province: string, district: string): boolean {
  return districtsOf(province).includes(district);
}
...
/**
 * "İstanbul / Esenyurt" — listelerde kullanılır. Açık adres tek satıra
 * sığmadığı için orada kısaltılmış metin yerine sadece bu gösterilir.
 */
export function formatRegion(parts: AddressParts): string | null {
  return [parts.customer_city, parts.customer_district].filter(Boolean).join(" / ") || null;
}

/** "Açık adres, İstanbul / Esenyurt" — boş alanlar atlanır, hiçbiri yoksa null. */
export function formatAddress(parts: AddressParts): string | null {
  const full = [parts.customer_address?.trim(), formatRegion(parts)]
    .filter(Boolean)
    .join(", ");
  return full || null;
}
